#summaries of data with arbitrary group subdivisions####

#' @title
#'
#' Obtain confidence intervals and related summary statistics for
#' a numeric variable.
#'
#' @description Obtain confidence intervals for the mean and/or median of a
...
#'
#' @export
describe_ci <- function(data, y, ..., ci_level = 0.95, mode = "p", sw_alpha = 0.05,
  ...
  if(interactive == FALSE){
    if (mode == "p") {
      ...
      suppressWarnings(
        describe_ci <- data %>%
          ...
          dplyr::summarise(n = sum(!is.na({{y}})),
                           na = sum(is.na({{y}})),
                           p50 = round(quantile({{y}}, probs = 0.50, na.rm = TRUE), 3),
                           IQR = round(IQR({{y}}, na.rm = TRUE), 3),
    ...
    if (mode == "p") {
      suppressWarnings(
        ...
        describe_ci <- describe_ci %>% DT::datatable(filter = "top", escape = FALSE,
                                      ...
                                      options = list(paging = paging,
                                                     autoWidth = TRUE,
                                                     columnDefs = list(list(className = 'dt-center', targets = c("box", "hist"))),
