...
`elucidate` provides a collection of convenience functions to make exploratory data analysis in R easier and more accessible for researchers to:
  ...
  * Interrogate data in search of row duplications and anomalous values with `copies()` and the `counts*` set of functions.
...
`copies()` can tell you how many rows are duplicated based on one or more variables (default is all of them). To return duplicated rows only we can set the filter argument to "dupes".
