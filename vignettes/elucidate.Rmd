...
knitr::opts_chunk$set(
  cache = TRUE, out.width = "85%", dpi = 300, fig.align = 'center'
)
```
...
```{r plot_histogram_basic, eval=FALSE}
pdata |> plot_histogram(y1)

```

![](figs/plot_histogram_basic.png)

Or for a violin plot of y1:

```{r plot_violin_basic, eval = FALSE}
pdata |> plot_violin(y1)
```

![](figs/plot_violin_basic.png)
...
```{r plot_histogram_grouped, eval = FALSE}
pdata |> 
  plot_histogram(x = y1, fill_var = g)
```

![](figs/plot_histogram_grouped.png)
...
```{r plot_histogram_grouped_alpha, eval = FALSE}
pdata |> 
  plot_histogram(x = y1, fill_var = g, 
                 alpha = 0.8) #80% opaque
```

![](figs/plot_histogram_grouped_alpha.png)
...
```{r plot_histogram_blue, eval = FALSE}
pdata |> plot_histogram(y1, fill = "blue2")
```

![](figs/plot_histogram_blue.png)

Similarly, to change the outline colour of the bars, we can use the "colour" argument.

```{r plot_histogram_blueblack, eval = FALSE}
pdata |> plot_histogram(y1, 
                         fill = "blue2",
                         colour = "black")
```

![](figs/plot_histogram_blueblack.png)
...
```{r plot_histogram_text, eval = FALSE}
pdata |> 
  plot_histogram(y1,
                 fill = "blue2",
                 colour = "black",
...
```{r, plot_bar_custom1, eval = FALSE}
pdata |> 
  plot_bar(fill_var = g)
```

![](figs/plot_bar_custom1.png)
...
```{r plot_bar_custom2, eval = FALSE}
pdata |> plot_bar(fill_var = g, position = "fill")

```

![](figs/plot_bar_custom2.png)
...
```{r plot_box_custom, eval = FALSE}
pdata |> 
  plot_box(y2, g, 
           outlier.colour = "red2")
```

![](figs/plot_box_custom.png)
...
```{r plot_density_custom, eval = FALSE}
pdata |> 
  plot_density(y1, 
               dnorm = TRUE)
```

![](figs/plot_density_custom.png)
...
```{r plot_violin, eval = FALSE}
pdata |> 
  plot_violin(y1, g, fill_var = g)
```

![](figs/plot_violin.png)
...
```{r plot_violin_quantiles, eval = FALSE}
pdata |> 
  plot_violin(y1, g, fill_var = g, 
              draw_quantiles = c(0.25, 0.5, 0.75))
```

![](figs/plot_violin_quantiles.png)
...
```{r plot_histogram_custom, eval = FALSE}
pdata |> 
  plot_histogram(y1, bins = 10)
```

![](figs/plot_histogram_custom.png)
...
```{r plot_histogram_dnorm, eval = FALSE}
pdata |> 
  plot_histogram(y1, 
                 binwidth = 10,
...
```{r plot_line_custom, eval = FALSE}
pdata |> 
  plot_line(y = y1, x = d, colour_var = g)
```

![](figs/plot_line_custom.png)
...
```{r ggplot2_messy_lines, eval = FALSE}
pdata |> 
...
```{r, bar_v_pie, eval = FALSE}
pdata |> plot_bar(fill_var = g)

```

![](figs/plot_bar_v_pie.png)
...
![](figs/plot_scatter_regression.png)
...
```{r plot_scatter_lm_grouped, eval = FALSE}
pdata |> 
  plot_scatter(y1, y2, 
               colour_var = g,
               regression_line = TRUE, regression_se = TRUE,
...
![](figs/plot_scatter_lm_grouped.png)
...
```{r plot_scatter_facetted, eval = FALSE}
pdata |> 
  plot_scatter(y1, y2, facet_var = g,
               jitter = TRUE, alpha = 0.7, size = 1)
```

![](figs/plot_scatter_facetted.png)
...
```{r plot_density_facetted, eval = FALSE}
pdata |> 
  plot_density(y1, facet_var = g)
```

![](figs/plot_density_facetted.png)
...
```{r plot_stat_error_point, eval = FALSE}
pdata |> 
...
![](figs/plot_stat_error_point.png)
...
```{r plot_raincloud, eval = FALSE}
pdata |> 
  plot_raincloud(y1, box_plot = TRUE)

```

![](figs/plot_raincloud.png)
...
```{r plot_raincloud_custom, eval = FALSE}
pdata |> 
  plot_raincloud(y1, 
                 box_plot = TRUE, 
                 coord_flip = TRUE,
...
![](figs/plot_raincloud_custom.png)
...
```{r plot_raincloud_custom_grouped, eval = FALSE}
pdata |> 
...
![](figs/plot_raincloud_custom_grouped.png)
...
```{r plot_var_nc, eval = FALSE}
pdata |> plot_var(y1, g)
```

![](figs/plot_var_nc.png)
...
```{r plot_var_pairs, eval = FALSE}
pdata |> 
  plot_var_pairs(cols = c("y1", "y2", "g"))
```

![](figs/plot_var_pairs.png)
...
```{r plot_var_pairs_grouped, eval = FALSE}
pdata |> 
  plot_var_pairs(cols = c("y1", "y2", "g"), 
                 group_var = high_low, basic = TRUE)
```

![](figs/plot_var_pairs_grouped.png)
...
```{r colour options, eval = FALSE}
colour_options()
```

![](figs/colour_options.png)
